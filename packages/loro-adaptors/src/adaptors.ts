...
export class LoroAdaptor implements CrdtDocAdaptor {
  ...
  constructor(doc?: LoroDoc, config: LoroAdaptorConfig = {}) {
    ...
    if (config.peerId !== undefined) {
      const pid =
        typeof config.peerId === "string"
          ? Number(config.peerId)
          : config.peerId;
    ...
    }
    {
      ...
      this.reachServerVersionPromise = { promise, resolve: resolve!, reject: reject! };
  ...
  applyUpdate(updates: Uint8Array[]): void {
    ...
    try {
      ...
      if (status.pending == null) {
        ...
        // The CRDT will handle merging them appropriately
      }
    } catch (error) {
...
export function createLoroAdaptorFromDoc(
  doc: LoroDoc,
  config: LoroAdaptorConfig = {}
): LoroAdaptor {
  return new LoroAdaptor(doc, config);
}

export function createLoroEphemeralAdaptor(
